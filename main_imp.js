...
const commands = [
    new SlashCommandBuilder()
        .setName('sd-base')
        .setDescription('Generate a new image using Stable Diffuse (base).')
        .addStringOption(option =>
            option.setName('prompt')
                .setDescription('Enter your prompt for image generation')
                .setRequired(true)),
    new SlashCommandBuilder()
        .setName('sd-dream')
        .setDescription('Generate a new image using Stable Diffuse (dream).')
        .addStringOption(option =>
            option.setName('dream-prompt')
                .setDescription('Enter your dream prompt for image generation')
                .setRequired(true)),
    new SlashCommandBuilder()
        .setName('complete')
        .setDescription('Completes your prompt.')
        .addStringOption(option =>
            option.setName('complete')
                .setDescription('Enter your prompt for completion')
                .setRequired(true)),
    new SlashCommandBuilder()
        .setName('clear')
    ...
    new SlashCommandBuilder()
        .setName('edit')
        .setDescription('Send a picture to edit.')
        .addStringOption(option =>
            option.setName('clear')
                .setDescription('Provide a mask and the image, then include a prompt.')
                .setRequired(true)),
    new SlashCommandBuilder()
        .setName('variation')
        .setDescription('Creates a variation of the image given.')
        .addStringOption(option =>
            option.setName('variation')
                .setDescription('Creats a variation of the image given.')
                .setRequired(true)),
    new SlashCommandBuilder()
        .setName('close')
        .setDescription('Used to close the private chats.'),
    new SlashCommandBuilder()
        .setName('chat')
...
client.on("interactionCreate", async (interaction) => {
    ...
    if ((commandName === 'close') && (interaction.channelId.startsWith('edit-') || interaction.channelId.startsWith('variation-') || interaction.channelId.startsWith('chat-'))) {
        try {
    ...
    switch (interaction.commandName) {
        ...
        case "sd-base":
            ...
            const basePrompt = interaction.options.getString('prompt');
            await handleBaseImageGeneration(interaction, basePrompt);
            break;
        ...
        case "sd-dream":
            ...
            const dreamPrompt = interaction.options.getString('dream-prompt');
            await handleDreamImageGeneration(interaction, dreamPrompt);
            break;
        ...
        case "complete":
            ...
            await handleHelpCommand(interaction, complete);
            break;
